...
class SeqTrackDecoder(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8,
        ...
        self.nhead = nhead
        self.bbox_head = MLP(d_model, d_model, bins + 2, 3)
        # confidence head: last-hidden -> scalar logit
    ...
    def forward(self, src, pos_embed, seq):
        ...
        # hs can be:
        #  - (num_layers, T, B, D)  if return_intermediate=True
        #  - (1, T, B, D)           if return_intermediate=False (unsqueezed)
        #  - (T, B, D)              in some rare cases
        if hs.dim() == 4:
            # take last layer's output -> (T, B, D)
            hs = hs[-1]
        # now hs is (T, B, D) -> permute to (B, T, D)
        hs = hs.permute(1, 0, 2)
